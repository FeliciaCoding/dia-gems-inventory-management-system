...
action (**action_id**, from_counterpart_id, to_counterpart_id, employee_id, terms, remarks, created_at, updated_at)
...
update_log (**log_sequence, action_id**, employee_id, update_type, old_value, new_value, log_time)
...
item (**lot_id**, stock_name, purchase_date, supplier_id, origin, responsible_office, created_at, updated_at, is_available)
    `supplier_id` references `counterpart.counterpart_id` NOT NULL


action_item(**action_id, lot_id**, quantity, unit_price, currency_code)
   `action_id` references `action.action_id`
...
return_memo_in (**action_id**, orig_memo_in_action_id, return_memo_in_num, back_date)
    `action_id` references `action.action_id`
    `orig_memo_action_id` references `memo_in.action_id` NOT NULL


return_memo_in_items( **action_id, lot_id**, notes)
    `action_id` references `return_memo_in.action_id`
    `lot_id` references `item.lot_id`
...
return_memo_out(**action_id**, orig_memo_action_id, return_memo_out_num, back_date)
    `action_id` references `action.action_id`
    `orig_memo_action_id` references `memo_out.action_id` NOT NULL


return_memo_out_items (**action_id, lot_id**, notes)
    `action_id` references `return_memo_out.action_id`
    `lot_id` references `item.lot_id`
...
back_from_lab_items (**action_id, lot_id**, notes)
    `action_id` references `back_from_lab.action_id`
    `lot_id` references `item.lot_id`


transfer_to_factory(**action_id**, transfer_num, ship_date)
...
back_from_factory_details(**action_id, lot_id**, after_weight_ct, after_shape, after_length, after_width, after_depth, weight_loss_ct, note)
    `action_id` references `back_from_factory.action_id`
    `lot_id` references `item.lot_id`
...
certificate(**certificate_id**, lot_id, lab_id, certificate_num, issue_date, shape, weight_ct, length, width, depth, clarity, color, treatment, gem_type, created_at, updated_at)
